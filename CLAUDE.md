...
- **The AI never invents a value.** Every extracted field is `.nullable()`, the schema carries a `confidence` enum, and the prompt says never guess. Prices always come from the database, never from the model. **Never add a `hotelName` field to the extraction schema**: an enquiry is addressed to the hotel and never names it, so the model would return the sender's company and every proposal would be filed under the buyer's name — plausibly, and therefore invisibly. The sender's company is extracted as `client_name`, which is what it is. `hotel_name` is the **tenant**: hotels use this product themselves, so with auth it comes from the session. It is the `SELLING_HOTEL` constant in `app/actions.ts`, not a form field — a text box would make a proposal's provenance whatever the seller typed. Replace the constant with the tenant lookup when auth exists; do not reintroduce the input.
